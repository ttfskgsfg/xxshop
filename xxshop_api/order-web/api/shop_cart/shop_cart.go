...
import (
	...
	"net/http"
	"strconv"
	"xxshop-api/order-web/api"
	"xxshop-api/order-web/forms"
...
func New(ctx *gin.Context) {
	...
	_, err := global.GoodsSrvClient.GetGoodsDetail(context.Background(), &proto.GoodsInfoRequest{
		Id: itemForm.GoodsId,
	})
	if err != nil {
	...
	rsp, err := global.OrderSrvClient.CreateCartItem(context.Background(), &proto.CartItemRequest{
		GoodsId: itemForm.GoodsId,
		UserId:  int32(userId.(uint)), //方法返回值是interface才能这样断言
		Nums:    itemForm.Nums,
	})
