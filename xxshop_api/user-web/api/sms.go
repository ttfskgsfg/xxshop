...
func GenerateSmsCode(witdh int) string {
	...
	r := len(numeric)
	rand.Seed(time.Now().UnixNano()) //随机当前时间
...
func SendSms(ctx *gin.Context) {
	...
	client, err := dysmsapi.NewClientWithAccessKey("cn-beijing", global.Serverconfig.AliSmsInfo.ApiKey,
	...
	request.QueryParams["SignName"] = "阿里云短信测试"                         //阿里云验证过的项目名 自己设置
	...
	//将验证码保存起来  redis存储验证码  手机号码 key 验证码 value
