...
import (
	...
	"xxshop_srvs/order_srv/model"
	"xxshop_srvs/order_srv/proto"
)
...
func (*OrderServer) CreateCartItem(ctx context.Context, req *proto.CartItemRequest) (*proto.ShopCartInfoResponse, error) {
	...
	//获取当前用户  Goods: req.GoodsId, User: req.UserId必须要有这个条件
	if result := global.DB.Where(&model.ShoppingCart{Goods: req.GoodsId, User: req.UserId}).First(&shopCart); result.RowsAffected == 1 {
		...
		//插入操作
		shopCart.User = req.Id
		shopCart.Goods = req.GoodsId
		shopCart.Nums = req.Nums
		shopCart.Checked = false
	}
	global.DB.Save(&shopCart) //save方法可以自动判断是更新还是插入 一定要传地址进去
...
func (*OrderServer) UpdateCartItem(ctx context.Context, req *proto.CartItemRequest) (*emptypb.Empty, error) {
	...
	var shopCart model.ShoppingCart
	//id是主键 直接first查询即可
	if result := global.DB.First(&shopCart, req.Id); result.RowsAffected == 1 {
...
func (*OrderServer) CreateOrder(ctx context.Context, req *proto.OrderRequest) (*proto.OrderInfoResponse, error) {
	/*
		新建订单
			1. 从购物车中获取到选中的商品
			2. 商品的价格自己查询 - 访问商品服务 (跨微服务) srv层之间调用
			3. 库存的扣减 - 访问库存服务 (跨微服务) srv层之间调用
			4. 订单的基本信息表 - 订单的商品信息表
			5. 从购物车中删除已购买的记录
	*/
	//查询当前购物车
	...
	if result := global.DB.Where(&model.ShoppingCart{User: req.UserId, Checked: true}).Find(&shopCarts); result.RowsAffected == 1 {
	...
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument, "批量查询商品信息失败")
	}
	var orderAmount float32
	var orderGoods []*model.OrderGoods
	var goodsInvInfo []*proto.GoodsInvInfo
	for _, good := range goods.Data {
		orderAmount += good.ShopPrice * float32(goodsNumsMap[good.Id]) //通过商品id查询商品购买数量
...
func (*OrderServer) OrderList(ctx context.Context, req *proto.OrderFilterRequest) (*proto.OrderListResponse, error) {
	...
	//global.DB.Where(&orders)  //后台管理的话 这样查询所有订单即可
	//如果传进来是0 int类型会被gorm忽略
	//即如果是后台管理系统 则userid为0，即不会走id为0的查询
	//result := global.DB.Where(&model.OrderInfo{User: req.UserId}).Find(&orders)
	//rsp.Total = int32(result.RowsAffected)
	//用count查询性能高些
	var total int64
...
func (*OrderServer) OrderDetail(ctx context.Context, req *proto.OrderRequest) (*proto.OrderInfoDetailResponse, error) {
	...
	//web层应该先查询一下订单id是否是当前用的
	//个人中心可以这么做 但是如果是后台管理系统，根据id来查，则会查不到结果
	//web层的话 如果判断是后台管理系统，只传递order的id，如果是电商系统还需要传递用户的id
	//底层的开发 尽量和业务减少关系
	//resutl := global.DB.Where(&order, req.Id)
	//主键查询id
